...
class Indez_Blog_PostController extends Mage_Core_Controller_Front_Action {
    ...
    public function viewAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (!Mage::getStoreConfig('blogconfig/comments/enabled')) {
            ...
            if (!$session->isLoggedIn() && Mage::getStoreConfig('blogconfig/comments/login')) {
                $session->addError($helper->__('You must be logged in to comment.'));
                if (!Mage::helper('blog/post')->renderPage($this, $identifier)) {
                    $this->_forward('NoRoute');
                }
                return;
            } else if ($session->isLoggedIn() && Mage::getStoreConfig('blogconfig/comments/login')) {
            ...
            try {
                ...
                if (Mage::getStoreConfig('blogconfig/recaptcha/enabled') && !$session->isLoggedIn()) {
                    $publickey = Mage::getStoreConfig('blogconfig/recaptcha/publickey');
                    $privatekey = Mage::getStoreConfig('blogconfig/recaptcha/privatekey');
                ...
                if (Mage::getStoreConfig('blogconfig/comments/approval')) {
                    $model->setStatus(2);
                    $session->addSuccess($helper->__('Your comment has been submitted.'));
                } else if ($session->isLoggedIn() && Mage::getStoreConfig('blogconfig/comments/loginauto')) {
                    $model->setStatus(2);
                    $session->addSuccess($helper->__('Your comment has been submitted.'));
                } else {
                    $model->setStatus(1);
            ...
            if (Mage::getStoreConfig('blogconfig/comments/recipient_email') != null && $model->getStatus() == 1 && isset($comment_id)) {
                ...
                try {
                    ...
                    $mailTemplate->setDesignConfig(array('area' => 'frontend'))
                            ->sendTransactional(
                                    Mage::getStoreConfig('blogconfig/comments/email_template'), Mage::getStoreConfig('blogconfig/comments/sender_email_identity'), Mage::getStoreConfig('blogconfig/comments/recipient_email'), null, array('data' => $postObject)
                    );
